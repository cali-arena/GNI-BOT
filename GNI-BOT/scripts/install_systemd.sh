...
# Override: APP_DIR=/opt/gni sudo bash scripts/install_systemd.sh
set -e

APP_DIR="${APP_DIR:-/opt/gni}"
UNIT_FILE=/etc/systemd/system/gni-bot.service

echo "=== GNI Bot — systemd install ==="
echo "  WorkingDirectory: ${APP_DIR} (override with APP_DIR=/opt/gni etc.)"
...
# Type=oneshot + RemainAfterExit (no Restart=) avoids "bad unit file" on all systemd versions.
# Containers use restart: unless-stopped in compose for 24/7.
cat > "$UNIT_FILE" << EOF
[Unit]
