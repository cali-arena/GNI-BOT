...
class User(Base):
    """App users: email + password_hash. One WhatsApp session per user."""
    __tablename__ = "users"
    ...
    whatsapp_session: Mapped[Optional["WhatsAppSession"]] = relationship(
        "WhatsAppSession", back_populates="user", uselist=False
    )


class WhatsAppSession(Base):
    """One WhatsApp session per user (unique user_id)."""
    __tablename__ = "whatsapp_sessions"
    __table_args__ = ()

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    user_id: Mapped[int] = mapped_column(ForeignKey("users.id", ondelete="CASCADE"), unique=True, nullable=False, index=True)
    status: Mapped[str] = mapped_column(String(32), default="disconnected", nullable=False)  # disconnected | qr_ready | connected
    session_path: Mapped[Optional[str]] = mapped_column(Text, nullable=True)  # e.g. /data/wa_sessions/user_<id>/
    ...
    user: Mapped["User"] = relationship("User", back_populates="whatsapp_session")
