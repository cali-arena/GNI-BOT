        ...
        if st.form_submit_button("Login"):
            ...
            if not e or not p:
                ...
                if code == 401:
                    ...
                    st.error("Server is not configured for login (JWT). Contact the administrator.")
                elif code == 404:
                    st.error("Login not available. This API only supports API-key auth. Set GNI_API_BASE_URL to an API that has email/password login (/auth/login).")
                elif err:
                    # Connection failed (code is None) often means mixed content or unreachable host
                    if code is None:
                        st.error("Cannot reach the server. Check the API URL; if this app is on HTTPS, the API must use HTTPS.")
                    else:
                        st.error(err)
                    ...
                    st.error("Login failed. This API may not support email/password login. Use an API that exposes POST /auth/login and returns access_token.")
    st.stop()
