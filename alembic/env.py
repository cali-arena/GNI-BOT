...
def get_url() -> str:
    """Get DATABASE_URL from env (via secrets provider if available). VM-first default: postgres:5432."""
    try:
        from apps.shared.config import DATABASE_URL_DEFAULT
        from apps.shared.secrets import get_secret
        return get_secret("DATABASE_URL", DATABASE_URL_DEFAULT)
    except Exception:
        return os.environ.get("DATABASE_URL", "postgresql://gni:gni@postgres:5432/gni")
